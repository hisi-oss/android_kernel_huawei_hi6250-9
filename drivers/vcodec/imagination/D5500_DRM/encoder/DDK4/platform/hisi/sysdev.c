...
void power_on_encode(int isDec)
{
    ...
    writel(0xe0000060,(ioremap(PERI_CRG,4) + 0x00));/*??a?????????????1???*/
    writel(0xe018,(ioremap(PERI_CRG,4) + 0x64));

    val = readl(ioremap(PMC,4) + 0x380);
    //val &= (~(0x1<<4));
    //writel(val,(ioremap(PMC,4) + 0x380));/*????VCODEC_SUBSYS???3?????1|o???????*/
	writel(0x00100000,(ioremap(PMC,4) + 0x380));/*????VCODEC_SUBSYS??????????????*/
    do
    {
    ...
    do
    {
        ...
        val &= (0x01<<4);
    }while(val);         /*???????VCODEC_SUBSYS??????3?????1|o???????*/

    if(isDec)
    {
        writel(0x04,(ioremap(PERI_CRG,4) + 0x150));/*MTCMOS??1???*/
        writel(0x800,(ioremap(PERI_CRG,4) + 0x30));/*?a??VENCo??NOC_VENC???????*/
        writel(0x04,(ioremap(PERI_CRG,4) + 0x148));/*ISO??????1???*/
        writel(0x200,(ioremap(PERI_CRG,4) + 0x88));/*?a?????*/

        val = readl(ioremap(PMC,4) + 0x380);
        val &= (~(0x1<<10));
	 writel(val,(ioremap(PMC,4) + 0x380));/*????NOC VDEC???3?????1|o???????*/
        do
        {
            val = readl(ioremap(PMC,4) + 0x384);
            val &= (0x01<<10);
        ...
        do
        {
            val = readl(ioremap(PMC,4) + 0x388);
            val &= (0x01<<10);
        }while(val);         /*?????PMC?DNOC_POWER_IDLEACK????0x384??? bit 10o??NOC_POWER_IDLE????0x388??? bit 10???a1???b0*/

    }
    else
    {
        writel(0x02,(ioremap(PERI_CRG,4) + 0x150));/*MTCMOS??1???*/
	    writel(0x400,(ioremap(PERI_CRG,4) + 0x30));/*?a??VENCo??NOC_VENC???????*/
	    writel(0x02,(ioremap(PERI_CRG,4) + 0x148));	/*ISO??????1???*/
        writel(0x100,(ioremap(PERI_CRG,4) + 0x88));/*?a?????*/

        //val = readl(ioremap(PMC,4) + 0x380);
        //val &= (~(0x1<<11));
        //writel(val,(ioremap(PMC,4) + 0x380));/*????NOC VENC???3?????1|o???????*/
		writel(0x08000000,(ioremap(PMC,4) + 0x380));/*????NOC VENC??????????????*/
        do
        {
            val = readl(ioremap(PMC,4) + 0x384);
            val &= (0x01<<11);
