...
#define VOTE_STAT_LOCK                  (1)   /*????????????????*/
#define VOTE_STAT_UNLOCK                (0)   /*????????????????*/
...
def type_id ??  ????len???? data???????? ??????????:

    TELE_MNTN_DEF_STR ??????data???????? ?? ????len????:
            U8 tmp[64]
            len = 64 Bytes

    TELE_MNTN_DEF_U8 ??????data???????? ?? ????len????:
            struct
            {
                U8 tmp0;
                U8 tmp1;
